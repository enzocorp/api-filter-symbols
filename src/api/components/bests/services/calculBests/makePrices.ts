...
function calculPrice(wantedQty : number|string, bookSide : orderbook['bids' | 'asks']) : number| string {
  ...
  let reste : number = +wantedQty //"reste" est le reste de BASE a acheter/Vendre pour atteindre l'objetif
...
function makeInfos (orderbook: orderbook, assets : Asset[],markets : Market[]):Price['infos'] {
  ...
  const asset_base  = assets.find(asset => asset.name === part[2])
  const asset_quote = assets.find(asset => asset.name === part[3])
  ...
  if(!asset_base){
    ...
    debug ("%s",`Le symbole "${orderbook.symbol_id}" sera ignoré car la valeur de son ASSET BASE est indéfinie (${part[2]}) ... !` )
  ...
  const infos : Price['infos'] = {
    ...
    quote_usd : asset_quote.price_usd, //Servira simplement a convertir la valeur du spread en usd
    base_usd : asset_base.price_usd, //Permettra de calculer la qté de "BASE" qui sera mise en jeux
    base : part[2],
    quote : part[3],
...
async function makeObject (orderbook : orderbook, assets : Asset[],markets : Market[]) : Promise<Price>{
  ...
  for (let usdQty = START_GRAPH; usdQty <= END_GRAPH; usdQty += PAS_GRAPH){
    ...
    isfor[usdQty] = {
      ...
      buy : calculPrice(qtyBase,orderbook.asks), //Prix d'achat en "QUOTE"
      sell : calculPrice(qtyBase,orderbook.bids) //Prix de vente en "QUOTE"
    }
  }
...
async function makePrices (orderbooks : orderbook[], assets : Asset[], markets : Market[]) : Promise<Price[]>{
  const promises : Promise<Price>[] = orderbooks.map(orderbook => makeObject(orderbook,assets, markets))

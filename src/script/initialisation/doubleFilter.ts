...
async function doubleFilter (symbols : Symbol[], assets : Asset[], markets : Market[],) :Promise<[Asset[], Market[]]> {
  const filterAssets = async ()=> assets.filter(asset => !!symbols.find(symb => symb.quote === asset.name || symb.base === asset.name))
  const filterMarkets = async ()=> markets.filter(market => !!symbols.find(symb => symb.market === market.name))
